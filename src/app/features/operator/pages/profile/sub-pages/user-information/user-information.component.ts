...
export class UserInformationComponent {
    ...
    userDetails$ = this.userInformationState.userDetails$;
    profilePictureUrl$ = this.userDetails$.pipe(
        map((userDetails) => {
        ...
        this.userContactForm.controls.roleId.valueChanges
            ...
            .subscribe((roleId) => {
                const defaultPartnerId =
                    this.userDetails$.getValue()?.partnerId;
    ...
    submitContactForm() {
        ...
        const currentUserDetails = this.userDetails$.getValue();
        const userDetails: UserDetailsConfig = {
            userId: '',
            nickName: userDetailFormItems.nickName ?? '',
            secondaryEmail: currentUserDetails?.secondaryEmail ?? '',
            roleId: selectedRole?.id ?? '',
            roleName: selectedRole?.name ?? '',
            isEmployee: currentUserDetails?.isEmployee ?? false,
        ...
        this.userInformationState
            ...
            .then(() => {
                const currentLinkedAgent =
                    this.userInformationState.userDetails$.getValue()
                        ?.partnerId;
